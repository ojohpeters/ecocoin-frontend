...
export async function getUserReferralCode(walletAddress) {
  try {
    console.log("Fetching user referral code:", walletAddress)

    const response = await fetch(`${API_BASE_URL}/user/referral_code?wallet=${walletAddress}`)
    ...
    const data = await response.json()
    console.log("User referral code data:", data)
...
// Claim airdrop - Updated to match new API
export async function claimAirdrop(walletAddress) {
  try {
    ...
    return {
      ...
      tokens: data.tokens || 1100,
      tx: data.tx || null,
      message: data.status || "Airdrop sent successfully",
    }
  } catch (error) {
...
export async function getAirdropStats() {
  try {
    ...
    const data = await response.json()
    console.log("Airdrop stats data:", data)

    return {
      wallets_registered: data.wallets_registered || 0,
      airdrops_sent: data.airdrops_sent || 0,
    }
  } catch (error) {
...
// Get user stats (total wallets) - Deprecated, use getAirdropStats instead
